                            ...
                            <a href="{{ url($slug . '/product/' . $item->slug) }}">
                                ...
                                @if ($item->Sub_image($item->id)['status'] == true)
                                    <img src="{{ asset($item->Sub_image($item->id)['data'][0]->image_path ?? '') }}"
                                        class="hover-img">
                                @else
                                    <img src="{{ asset($item->Sub_image($item->id)) }}" class="hover-img">
                                @endif
                                ...
                                @if ($item->variant_product == 0)
                                    <div class="price">
                                        <ins>{!! currency_format_with_sym(
                                            \App\Models\Product::ProductPrice($currentTheme, $slug, $item->id, $item->variant_id),
                                            $store->id,
                                            $currentTheme,
                                        ) !!}<span class="currency-type">
                                                {{ isset($tax_option['price_suffix']) ? $tax_option['price_suffix'] : '' }}</span></ins>
                                    </div>
                                @else
                                    <div class="price">
